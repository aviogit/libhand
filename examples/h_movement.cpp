...
void move_joint(joint_names joint, double radians, InputData & input_data, int delay_ms = 70)
{
    input_data.hand_pose.bend(joint) = radians;
    input_data.hand_renderer.SetHandPose(input_data.hand_pose);
...
void move_joint_deg(joint_names joint, uint16_t degrees, InputData & input_data, int delay_ms = 70)
{
    double radians = 3.14169 * degrees / 180.0;
    move_joint(joint, radians, input_data, delay_ms);
}

void move_joint_deg_to(joint_names joint, int16_t degrees, InputData & input_data, int delay_ms = 70)
{
    double curr_radians = input_data.hand_pose.bend(joint);
    double radians = 3.14169 * degrees / 180.0;

    ///std::cout << "Going from " << curr_radians << " to " << radians << std::endl;
    ...
    while (true)
    {
        move_joint(joint, curr_radians + increment, input_data, delay_ms);
        curr_radians = input_data.hand_pose.bend(joint);
        ///std::cout << "I'm at " << curr_radians << " of " << radians << std::endl;
        if (go_positive)
        {
            if (curr_radians >= radians - threshold)
                break;
        }
        else
        {
...
int main (int argc, char **argv)
{
	...
	try
	{
...
#if 0
        auto pose = pose_list.at(0);
        show_pose(pose, input_data);
#endif

        joint_names curr_joint = joint_names::arm;
        //move_joint_deg(curr_joint, 200, input_data);
        move_joint_deg_to(curr_joint, 200, input_data, 10);

        uint32_t joint_counter = 0;
        while (joint_counter < arm)
        {
            move_joint_deg_to(joint_names(joint_counter), -20, input_data, 10);
            joint_counter++;
#if 0
            hand_pose.bend (joint_counter++) += 3.14159 / 2;
            input_data.hand_renderer.SetHandPose (hand_pose);
        ...
        {
            joint_counter = 0;
            while (joint_counter < wrist)
            {
                move_joint_deg_to(joint_names(joint_counter), 20 * (iterations % 2 == 0 ? 1 : -1), input_data, 5);

                ///std::thread joint_thread(move_joint_deg_to, joint_names(joint_counter), 20 * (iterations % 2 == 0 ? 1 : -1), std::ref(input_data), 5);
